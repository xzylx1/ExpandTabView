...
{
    public interface OnItemClickListener {                                           //提供item单击回调接口
    ...
    private int selectedPos = -1;                                                    //保存item的Selected状态
    private String selectedText = "";                                                //点击获取到条目的名称
    private Drawable normalDrawbleId;                                                //item的背景
    private Drawable selectedDrawble;                                                //item的checked图
    private float textSize;                                                          //设置列表的字体大小
    private OnClickListener onClickListener;                                         //自带条目点击事件监听
    private OnItemClickListener mOnItemClickListener;                               //自定义条目点击事件监听
    ...
    public TextAdapter(Context context, List<String> listData, int sId, int nId) {
        ...
        normalDrawbleId =  mContext.getResources().getDrawable(nId);
        init();
    }
    ...
    private void init() {
        onClickListener = new OnClickListener() {                                     //设置每个条目的监听
            ...
            public void onClick(View view) {
                Log.e("onClick", "onClick: 00000" );
                selectedPos = (Integer) view.getTag();                                //自动获取到被点击条目的id标签并保存
                setSelectedPosition(selectedPos);                                     //设置item的Selected状态
                if (mOnItemClickListener != null) {
                    //当被点击且绑定了PopWindow 回调PopView的实现的方法 回传数据参数
     ...
     * 获取选中的position
     */
    public int getSelectedPosition() {                                                       //用于获取被选内容位置
    ...
    @Override
    public View getView(int position, View convertView, ViewGroup parent) {                      //实时重绘item
        TextView view;
        if (convertView == null) {
        ...
        if (selectedText != null && selectedText.equals(mString)) {
            ...
            view.setBackgroundDrawable(normalDrawbleId);                                          //设置未选中状态背景图片
        }
